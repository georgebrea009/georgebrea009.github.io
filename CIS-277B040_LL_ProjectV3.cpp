#include <iostream>
#include <string>
using namespace std;

// Node structure
struct Node;
typedef Node* NodePtr;
struct Node {
    char sname[30];
    int sid;
    float sgpa;
    int data;
    NodePtr next;
};

// Function to create a list if one does not exist
void createList(Node*& head) 
{
    if (head == nullptr) {
        head = new Node();
        head->data = 0; // Initial value, can be changed as needed
        head->next = nullptr;
        cout << "List created with initial node value: " << head->data << endl;
    } else {
        cout << "List already exists!" << endl;
    }
}

// Function to insert a node at the beginning of the linked list
void addNode(Node*& head) 
{
    if (head == nullptr) {
        cout << "List does not exist. Please create a list first." << endl;
        return;
    }
    /*Node* newNode = new Node();
    newNode->data = 0;
    newNode->next = head;
    head = newNode;*/
    Node *temp, *temp2; // Temporary pointers
    Node* newNode = new Node();
    cin.ignore(); // To ignore the newline character left in the input buffer
    cout << "Please enter the name of the student: ";
    cin.getline(newNode->sname, 30);
    cout << "Please enter the student's ID? (4 digit number between 1000-9999) ";
    cin >> newNode->sid;
    while (newNode->sid < 1000 || newNode->sid > 9999) 
    {
        cout << "You entered an invalid number. Enter a 4 digit student ID: " << endl;
        cin >> newNode->sid;
    }
    cout << "Please enter the student's GPA: ";
    cin >> newNode->sgpa;
    while (newNode->sgpa < 1 || newNode->sgpa > 5) 
    {
        cout << "You entered an invalid GPA. Enter a 2 digit with a dot in the middle ID: " << endl;
        cin >> newNode->sgpa;
    }
    newNode->next = nullptr;
          // Set up link to this node
             if (head == nullptr) 
                 head = temp;
            else
                 {
                    temp2 = head; // We know this is not NULL - list not empty!   
                   while (temp2->next != nullptr) 
                      {
                        temp2 = temp2->next; // Move to next link in chain
                       } 
                   temp2->next = temp; // makes the last node point to the new node which points to Null
                 }


}

// Function to delete a node from the linked list
void deleteNode(Node*& head, int value) 
{
    if (head == nullptr) {
        cout << "List is empty!" << endl;
        return;
    }
    if (head->data == value) {
        Node* temp = head;
        head = head->next;
        delete temp;
        return;
    }
    Node* temp = head;
    while (temp->next != nullptr && temp->next->data != value) {
        temp = temp->next;
    }
    if (temp->next == nullptr) {
        cout << "Value not found in the list!" << endl;
    } else {
        Node* nodeToDelete = temp->next;
        temp->next = temp->next->next;
        delete nodeToDelete;
    }
}

// Function to display the linked list
void displayList(Node* head) 
{
    if (head == nullptr) {
        cout << "List is empty!" << endl;
        return;
    }
    Node* temp = head;
    cout << "Linked list: ";
    while (temp != nullptr) {
        cout << temp->data << " ";
        temp = temp->next;
    }
    cout << endl;
}

// Function to delete the entire linked list
void deleteList(Node*& head) 
{    
    if (head == nullptr) {
        cout << "There is no list." << endl;
        return;
    }
    Node* current = head;
    Node* nextNode;
    while (current != nullptr) {
        nextNode = current->next;
        delete current;
        current = nextNode;
    }
    head = nullptr;
    cout << "The list has been deleted." << endl;
}

int main() 
{
    Node* start_ptr = nullptr;
    int choice, value;

    do {
        cout << "1. Create list" << endl;
        cout << "2. Add a node" << endl;
        cout << "3. Delete node" << endl;
        cout << "4. Display the list" << endl;
        cout << "5. Delete entire list" << endl;
        cout << "6. Exit" << endl;
        cout << "Enter your choice: ";
        cin >> choice;

        switch (choice) {
            case 1:
                createList(start_ptr);
                break;
            case 2:
                addNode(start_ptr);
                break;
            case 3:
                cout << "Enter the value to delete: ";
                cin >> value;
                deleteNode(start_ptr, value);
                break;
            case 4:
                displayList(start_ptr);
                break;
            case 5:
                deleteList(start_ptr);
                break;
            case 6:
                cout << "Exiting..." << endl;
                break;
            default:
                cout << "Invalid choice!" << endl;
        }
    } while (choice != 6);

    return 0;
}